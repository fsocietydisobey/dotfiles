# /inbox [session_id_or_name] — read pending notes from a khimaira session

Manual check of an inbox. Default = your current session. Pass an optional session_id or friendly name (e.g. `/inbox khimaira-builder`) to peek at a different session's inbox without affecting your own.
...
2. Call `mcp__khimaira__session_pending_notes(session_id=<id>, mark_read=true)`.
3. Render the result:
