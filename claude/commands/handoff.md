...
- A **khimaira-attached project label** (e.g. `backend`, `khimaira`) — the readable, stable name from `khimaira attach`. **Preferred.**
- An absolute filesystem path (e.g. `/home/user/work/foo`) — fallback for projects not attached.

You usually want the label form. Run `khimaira attached` if you forget the labels.
...
The handoff:
- Stays in `~/.local/state/khimaira/handoffs.jsonl` for 7 days by default (`expires_in_hours=168`).
...
1. Parse `$ARGUMENTS`:
   - First whitespace-separated token = `<project_or_path>`. Detect type:
     ...
     - Otherwise, treat as a khimaira-attached project label → pass as `scope_project`.
   ...
   - If `$ARGUMENTS` is empty or has one token only, render:
     ...
     Examples:
       ...
       /handoff khimaira "shipped tasks; pickup at workspaces/IMPLEMENTATION.md"
       /handoff /home/user/work/foo "for any session in /home/user/work/foo"
     Run `khimaira attached` to see project labels.
     ```

2. Resolve your own session id (sender). Use SessionStart hook value; fall back to `mcp__khimaira__session_list`.

3. Call `mcp__khimaira__session_post_handoff(from_session_id=<my_id>, text=<message>, scope_project=<label>)` if a label was given; otherwise pass `scope_cwd=<path>`.
