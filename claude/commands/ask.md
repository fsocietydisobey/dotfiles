...
Asks another chimera session a question and waits for the answer in the same turn. Wraps the two-step pattern `mcp__chimera__session_log_question(target=...)` + `mcp__chimera__session_wait_for_answer(...)` so the user doesn't have to remember both.
...
Default timeout: 300 seconds. Override with `--timeout <seconds>` (e.g. `/ask --timeout 60 chimera-builder "..."`).
...
2. Resolve your own session id (sender). Use the value the SessionStart hook surfaced; fallback to `mcp__chimera__session_list` if unknown.

3. Call `mcp__chimera__session_log_question(session_id=<my_id>, text=<question>, target_session_id=<target>)`. Capture the returned question id.

4. Call `mcp__chimera__session_wait_for_answer(session_id=<my_id>, question_id=<id>, timeout=<timeout>)`.
...
- If you don't need the answer in the SAME turn (just want to leave a question), use `mcp__chimera__session_log_question` directly — it's the non-blocking version.
