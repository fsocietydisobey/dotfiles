# /khimaira-chain — Kick off khimaira's SPR-4 pipeline
...
1. Call `mcp__khimaira__chain_pipeline` with:
   - `task_description`: $ARGUMENTS
   - `context`: empty unless the user provided extra context
2. Capture the returned `job_id` and `thread_id`.
3. Tell the user: "Started job `<job_id>`. Track progress with `mcp__khimaira__status('<job_id>')`. When the pipeline pauses for review, run `mcp__khimaira__approve('<job_id>')` to continue."
...
- For batch ops (fix all X, add Y to N modules), use `/khimaira-swarm` instead.
