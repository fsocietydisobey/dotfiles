# /chimera-architect — Plan an implementation via chimera

Spawn chimera's architect (Claude Code under the hood) to design an implementation plan and write `IMPLEMENTATION.md` + `TODO.md` into the current project.
...
2. Call `mcp__chimera__architect` with the resolved `goal` / `context` / `constraints` plus `cwd` = the current working directory.
