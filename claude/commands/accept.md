# /accept <targets> [feedback] — master confirms agent results

After `/delegate` returns, master inspects each agent's answer and decides. `/accept` is the positive-ack path: the work is good, no rework needed, just close the loop with the agent so they know.

## Args

```
/accept agent-1,agent-3 "good citations, merging"
/accept agent-2
```
...
- If you don't `/accept`, the agent has no way to know whether their work was used. For multi-round work blocks, that breeds the "did master ever see my answer?" question.
...
- To reject + give rework context, use `/reject` instead.
