# /khimaira-swarm — Parallel batch ops via khimaira (PDE)
...
1. Call `mcp__khimaira__swarm` with:
   - `goal`: $ARGUMENTS
...
3. Tell the user: "Started swarm `<job_id>` with budget $2.00 / max 10 workers. Track via `mcp__khimaira__status('<job_id>')`."
...
- For single, complex tasks → `/khimaira-chain`. For autonomous loops → `/khimaira-refiner`.
