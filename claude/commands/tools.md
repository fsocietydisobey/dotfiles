# /tools — list everything chimera exposes
...
1. Run `chimera tools` via Bash. Optionally pass a substring filter as `$ARGUMENTS` (e.g. `/tools session` runs `chimera tools session`).
