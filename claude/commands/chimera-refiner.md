# /chimera-refiner — Autonomous codebase refinement loop (CLR)
...
2. Call `mcp__chimera__chain_refiner` with the parsed (or default) values.
3. Capture the returned `job_id`.
4. Tell the user: "Refiner started — job `<job_id>`, max cycles X, budget $Y. It will loop autonomously until convergence/budget exhaustion. Track via `mcp__chimera__status('<job_id>')`."
