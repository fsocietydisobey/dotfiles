# /reject <targets> <rework-context> — master sends an agent back for rework

After `/delegate` returns, if an agent's answer wasn't right (wrong file, missed context, incorrect citation, etc.), master uses `/reject` to send the agent a NEW targeted question with rework context. The agent then answers the new question; master can `/delegate` again or just `session_wait_for_answer`.

## Args

```
/reject agent-2 "missed the auth/ feature — focus there, ignore everything else"
/reject agent-1,agent-4 "wrong codebase, you're in jeevy_portal not khimaira"
```
...
2. **Validate** — if no rework context after the targets, render:
   ```
   Usage: /reject <agent1,agent2,...> <rework-context>
...
5. **Render**:
   ...
   `/delegate` to fan them out alongside other work.
   ```
...
- Master does NOT block here — `/reject` fires the questions and returns. Use `/delegate` if you want to await the new answers, or `session_wait_for_answer` on the new question ids returned by this command.
- Don't `/reject` without rework context — agents have no way to infer what to change. Vague reject = vague rework.
